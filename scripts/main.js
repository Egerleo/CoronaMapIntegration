...
* My update **/
  ...
  map.on('click', function (evt) {
    ...
    var feature2 = map.forEachFeatureAtPixel(evt.pixel, function (feature2) {
      return feature2;
    })
    var countryName = feature2.values_.name
    ...
    if (countryName === "Republic of Serbia") {
...
function fetchTotalData() {
  ...
  fetch("https://pomber.github.io/covid19/timeseries.json") //request data asynchronously
    ...
    .then(function (data) {
      ...
      //see wether the arrow should point up or down
      function createArrow (results){
        if (results >= 0){
          arrow =  '&#8593; &nbsp;';
        }
        else if (results < 0){
      ...
      var arrowConfirmed = createArrow(resultsConfirmedIncrease);
      var arrowActive = createArrow(resultsActiveIncrease);
      var arrowRecovered = createArrow(resultsRecoveredIncrease);
      var arrowDead = createArrow(resultsDeadIncrease);

    
      document.getElementById("totalStats").innerHTML = ("<table><th>Global </th><th>Cases</th></th><th>Increase</th>"
        + "<tr><td>Confirmed: </td><td>" + resultsConfirmed + "</td><td class='globalPlus'>"+ arrowConfirmed + resultsConfirmedIncrease + "</td></tr>"
        + "<tr><td>Active:    </td><td>" + resultsActive + "</td><td class='globalPlus'>" + arrowActive + resultsActiveIncrease + "</td></tr>"
        + "<tr><td>Recovered: </td><td>" + resultsRecovered + "</td><td class='globalPlus'>"+ arrowRecovered + resultsRecoveredIncrease + "</td></tr>"
        + "<tr><td>Dead:      </td><td>" + resultsDead + "</td><td class='globalPlus'>"+ arrowDead + resultsDeadIncrease + "</td></tr></table> "
      );
